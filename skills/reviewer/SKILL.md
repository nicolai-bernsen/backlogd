...
**standards corpus** — the Accepted standards under `docs/standards/` (an Accepted standard
is a hard rule; violating one is `❌`, same weight as a failed DoD line). The corpus has two
kinds, indexed by the same machinery: framework **ADRs** (`docs/standards/adrs/ADR-*.md` —
backlogd's own architectural decisions) and **per-engagement standards**
(`docs/standards/engagement/STD-*.md` — the domain rules for *this* instance's product, the
"domain DoD" half of ADR-004's *value = specialists × standards*; NB-400). The risk NB-380
exists to defuse: if "consult the standards" meant *load all the prose into context every
review*, the verification layer would get slow, burn the token/context budget (the NB-379
quota pressure), and — worst — a reviewer swimming in N prose standards would *miss* the one
that applies. With a corpus that grows over time, that failure gets worse, not better.

**The fix is the authoring format, not a storage backend.** Each standard carries
machine-readable front-matter — a crisp **checkable `assertion`**, an **`applies-to`** scope
(`domains` / `file-patterns` / `decision-types`), a lifecycle `status`, and — for a
multi-rule per-engagement standard — an optional **`rules`** block (numbered `R1`, `R2`, …,
each a `level | assertion | fix` triple) — and `scripts/standards_index.py` generates a
single small **committed** artifact, `docs/standards/index.json`, from it. The reviewer's
load order (wired into `agents/reviewer.md`):
...
2. **Filter to applicable standards by `applies-to`** — match the diff's changed files
   ...
   (not yet binding), `Superseded`, and `Deprecated` (history). Standards stay agile:
   Accepted today is a hard rule but reconsiderable — reopen-and-supersede per the template
   lifecycle, so the reviewer enforces only what is *currently* Accepted. Most standards are
   irrelevant to any given diff.
3. **Judge against each applicable `assertion`** — usually enough to call met/unmet. **For a
   standard carrying a `rules` array**, walk its rules: a **`MUST`** rule the change trips is
   a hard block, cited **by its rule id and its `fix`** (e.g. "🚫 STD-001 R2 — paid set from
   the browser redirect; fix: set paid only from the verified webhook") — never a bare
   "non-compliant"; a **`SHOULD`** rule is advisory (flag, don't block alone).
4. **Open a full standard only when the rationale is needed** — never the whole set.
...
3: the reviewer cites *which* indexed standards were applicable (and *which rule id* for a
per-engagement rule) and how the diff met them.
...
> **Read this section before** adding a standard to `docs/standards/adrs/` (an ADR) or
> `docs/standards/engagement/` (a per-engagement `STD-*.md`), or changing how the reviewer
> consults it. Front-matter is the single source of truth: edit it, then regenerate the
> index (`python scripts/standards_index.py`) in the same change — CI's drift test enforces
